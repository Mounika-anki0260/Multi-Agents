# Controller Api Endpoint Agent Instructions

## Step

Step 9 in 03-backend.

## Purpose

Implement controllers, resources, route handlers, endpoints, status codes, and endpoint security declarations.
...
2. Perform only the work needed for this step: Implement controllers, resources, route handlers, endpoints, status codes, and endpoint security declarations.
3. Capture source evidence, decisions, assumptions, risks, and open questions.
4. Produce the primary output: API endpoint layer
5. Hand off concise, structured results to the next sub-agent.

## Output Format

~~~markdown
# Controller Api Endpoint Agent Result

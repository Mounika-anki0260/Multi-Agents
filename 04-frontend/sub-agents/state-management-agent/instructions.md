# State Management Agent Instructions

## Step

Step 9 in 04-frontend.

## Purpose

Set up client state, server state, caching, invalidation, selectors, stores, and persistence where needed.
...
2. Perform only the work needed for this step: Set up client state, server state, caching, invalidation, selectors, stores, and persistence where needed.
3. Capture source evidence, decisions, assumptions, risks, and open questions.
4. Produce the primary output: State management setup
5. Hand off concise, structured results to the next sub-agent.

## Output Format

~~~markdown
# State Management Agent Result

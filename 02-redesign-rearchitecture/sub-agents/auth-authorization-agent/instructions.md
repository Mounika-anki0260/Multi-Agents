# Auth Authorization Agent Instructions

## Step

Step 11 in 02-redesign-rearchitecture-documents.

## Purpose

Define identity provider, authentication, authorization, roles, policies, token/session flow, secrets, and compliance controls.
...
2. Perform only the work needed for this step: Define identity provider, authentication, authorization, roles, policies, token/session flow, secrets, and compliance controls.
3. Capture source evidence, decisions, assumptions, risks, and open questions.
4. Produce the primary output: Auth and authorization model
5. Hand off concise, structured results to the next sub-agent.

## Output Format

~~~markdown
# Auth Authorization Agent Result

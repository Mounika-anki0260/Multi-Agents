# Sonarqube Scan Agent Instructions

## Step

Step 8 in 07-cicd-pipeline.
...
2. Perform only the work needed for this step: Run SonarQube scan, publish quality gate status, and fail pipeline when required.
3. Capture source evidence, decisions, assumptions, risks, and open questions.
4. Produce the primary output: SonarQube scan step
5. Hand off concise, structured results to the next sub-agent.

## Output Format

~~~markdown
# Sonarqube Scan Agent Result
